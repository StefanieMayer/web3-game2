import { Box, SimpleGrid, Card, CardHeader, Heading, Input , Text,     Table,
    Tbody,
    Td,
    Th,
    Thead,
    Tr, Button, Image, Checkbox } from "@chakra-ui/react";
import styles from "../styles/Home.module.css";
import Authentication from "../components/Authentication";
import Head from "next/head";
import woddsABI from "../ABI/wordsNFT.json";
import { ethers } from "ethers";
import { useWeb3Auth } from "../context/web3auth";
import { useEffect, useState } from "react";
import Web3 from "web3";
import Modal from 'react-modal';
import { FaSpinner } from 'react-icons/fa';
import { Center, Square, Circle } from '@chakra-ui/react'
import { faker } from '@faker-js/faker'
 

const customStyles = {
    content: {
        top: '30%',
        left: '50%',
        right: 'auto',
        bottom: 'auto',
        marginRight: '-50%',
        transform: 'translate(-50%, -50%)',
        backgroundColor: 'rgba(0, 0, 0, 0.75)'
    }
};


export default function Mintpage(props) {
    const { web3Auth, provider, isLoading, user, createContract, getAccounts, getBalance, pureProvider } = useWeb3Auth()
    const [localProvider, setProvider] = useState(null);
    const [userAddress, setUserAddress] = useState(user)
    const [show, setShow] = useState(false)
    const [modalIsOpen, setIsOpen] = useState(false);
    const [txHash, setTxHash] = useState('');
    const [loadingTX, setloadingTX] = useState(false);

    const { isAuth, role } = props;
    const [data, setData] = useState([]);

    useEffect(() => {
        const data = new Array(100).fill().map(() => ({
            user: faker.internet.userName(),
            address: faker.finance.ethereumAddress(),
            score: faker.finance.amount()
        }));
        data.sort((a, b) => parseFloat(b.score) - parseFloat(a.score));

        setData(data);
    }, []);

    useEffect(() => {
        setUserAddress(user)
    }, [user])

    async function mintNFT(_localProvider) {
        if (!provider || loadingTX) {
            return;
        }
        try {
            //console.log("bal", await getBalance());
            const bal = await getBalance();
            if (bal < 10000000000000000) {
                alert("Insufficient balance");
                return;
            }
            let woddsNFT = await createContract(woddsABI.abi, woddsABI.address, userAddress)
            setloadingTX(true)
            //const web3 = new Web3(_localProvider);
            // let woddsNFT = new web3.eth.Contract(woddsABI.abi, woddsABI.address)
            let tx = await woddsNFT.methods.payToMint().send({ from: userAddress, value: '10000000000000000' });
            console.log(tx)
            setTxHash(`https://goerli.etherscan.io/tx/${tx.transactionHash}`);
            // Open the modal
            setIsOpen(true);
        } catch (err) {

        } finally {
            setloadingTX(false)
        }
        // console.log(userAddress)



    }



    const closeModal = () => {
        // Close the modal and reset txHash state
        setIsOpen(false);
        setTxHash('');
    }

    return (
        <div className={styles.container}>
            <Authentication isAuth={isAuth} />

            <Head>
                <title>Campaign Donation</title>
                <meta name="description" content="Generated by create next app" />
                <link rel="icon" href="/favicon.ico" />
            </Head>
            <div className={styles.container}>
                <main className={styles.main}>
                    <Box className={"max-w-4xl min-w-600"}            >
                        <SimpleGrid spacing={6} row={1} columns={2}>
                            <Box>   <Card _hover={{ transform: "scale(1.0)" }}
                                transition="all 0.2s ease-out"
                                display="inline-block"
                                overflow="hidden" >
                                <CardHeader>
                                    <Center>
                                        <Image borderRadius='10%' src={'/campaignImg1.png'} boxsize='200px' alt='placeholder' />
                                    </Center>
                                    <Center>
                                        <Heading size='md' className={"my-4"}> Proof of Donation  : ERC-1155 Campaign NFT</Heading>
                                    </Center>
                                </CardHeader>

                            </Card>
                            </Box>
                            <Box>
                            <Text as='b' fontSize='lg' >VAKA Health Foundation</Text> <br></br> <br></br>
                            <Text fontSize='lg' >The VAKA Foundation builds a social enterprise for Healthcare workers.</Text> <br></br>
                            <Text fontSize='lg' >They provide a medical professional education to ensure universal accessibility and effective communication on trending healthcare matters. </Text>
                            <br />
                            <br />
                            <br />

                            <Input placeholder='Enter Donor`s name ' />
                            <Button disabled={loadingTX} onClick={() => mintNFT(localProvider)}

                                style={{ marginTop: '140px' }} className={"max-w-4xl my-10"}> {!loadingTX && <div className="loader"> Support Campaign with (0.01 ETH)</div>}
                                {loadingTX && <svg aria-hidden="true" class="inline w-10 h-10 mr-2 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z" fill="currentColor" />
                                    <path d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z" fill="currentFill" />
                                </svg>}
                            </Button> </Box>

                        </SimpleGrid>
                        <Modal isOpen={modalIsOpen} onRequestClose={closeModal} style={customStyles}>
                            <h2>Transaction Hash:</h2>
                            <br />
                            <a href={txHash} target={"_blank"}><u>{txHash}</u></a>
                            <br />
                            <br />
                            <a href={`https://testnets.opensea.io/${userAddress}?search[collections][0]=unidentified-contract-30309`} target={"_blank"}><u>View on OpenSea</u></a>
                            <br />
                            <br />
                            <button onClick={closeModal}>Close</button>
                        </Modal>
                        <Box padding={4}>
                        <br></br>

                            <Heading fontSize={{ base: '2xl', md: '3xl' }}>List of Donors </Heading>
                            <br></br>
                            <br></br>

                            <Box overflowY="auto" maxHeight="500px" className={"max-w-4xl"}>
                                <Table variant="striped" colorScheme="teal">
                                    <Thead position="sticky" top={0} bgColor="grey" zIndex={1}>
                                        <Tr>
                                             <Th>Name</Th>
                                            <Th>Address</Th>
                                        </Tr>
                                    </Thead>
                                    <Tbody>
                                        {data.map(({ user, address, score }, index) => (
                                            // eslint-disable-next-line react/jsx-key
                                            <Tr
                                                overflow="clip">
                                                 <Td>{user}</Td>
                                                <Td>{address}</Td>
                                            </Tr>
                                        ))}
                                    </Tbody>
                                </Table>
                            </Box>
                        </Box>
                    </Box>
                </main>
            </div>


        </div>
    );
}